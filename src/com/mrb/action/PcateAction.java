...
public class PcateAction extends Action {
	...
	public ActionForward execute(ActionMapping mapping, ActionForm form,
		...
		if ("list".equals(act)) { // 获取分类列表 for ajax
			...
			ArrayList<PcateBean> ulist = bs.getPcateList();
			log.debug("ulist.size(): " + ulist.size());
			req.setAttribute("ulist", ulist);

			StringBuilder html = new StringBuilder(
					"<table class=\"table\"><thead><tr><th>分类ID</th><th>分类名</th><th>注册日期</th><th style=\"width: 26px;\"></th></tr></thead><tbody>");
			if (ulist != null && ulist.size() > 0) {
				for (int i = 0; i < ulist.size(); i++) {
					...
					html
							.append("<a href=\"#myModal\" role=\"button\" data-toggle=\"modal\" onclick=\"setcid(this);\"><i class=\"icon-remove\"></i></a></td></tr>");
